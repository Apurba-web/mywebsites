...
*/
      if($error_cnt == 0) {
        ...
        $sql=$db->prepare("UPDATE GRN SET GRDate = ?, i_code = ?, s_code = ?, Driver = ?, TicketNo = ?, UOM = ?, Qty = ?, BagWt = ?, Rate = ? WHERE GRN = ?");

        $sql->bind_param("ssssssidds", $GRDate, $ItemCode, $SupplierCode, $Driver, $TicketNumber, $UOM, $Quantity, $BagWt, $Rate, $GRN);
