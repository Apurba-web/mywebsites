...
echo $ItemType;
      ...
      $sql=$db->prepare("UPDATE item SET i_name = ?, i_type =?, i_uom_primary=?, i_uom_secondary = ?, i_conversion_factor=?, i_dimension=?, i_dimension_unit=?, i_gst_code=?, i_gst_percentage=? WHERE i_code = ?");

      $sql->bind_param("ssssdissds", $ItemName, $ItemType, $ItemUnitPrim, $ItemUnitSec, $ConversionFactor, $Dim, $DimUnit, $GSTCode, $GSTPer, $ICode);
