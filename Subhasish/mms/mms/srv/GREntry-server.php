  ...
  if(isset($_POST['submit'])) {
    try {
      ...
      $GRN = $db -> real_escape_string("0001");
...
*/
      if($error_cnt == 0) {
        ...
        $sql=$db->prepare("SELECT MAX(srl) AS LastSrl FROM GRN");
        ...
        $sql=$db->prepare("INSERT INTO GRN (GRN, GRDate, i_code, s_code, Driver, TicketNo, UOM, Qty, BagWt, Rate) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $sql->bind_param("sssssssddd", $GRN, $GRDate, $ItemCode, $SupplierCode, $Driver, $TicketNumber, $UOM, $Quantity, $BagWt, $Rate);
